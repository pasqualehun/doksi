...
A \figref{class} ábrán láthatóak a programban használt osztályok. A \textit{Program} statikus osztály a main függvényt, mint belépési pontot tartalmazza. Ez példányosítja az \textit{Instruments} osztályt, mely a különbözõ \textit{View}-ok megjelenítéséért felelõs. Az Altimeter, Compass, Speed, Vario a mûszereket megvalósító osztályok, a PlanView a tervezõképernyõ, az ErrorOverview osztály a diagnosztikai képernyõ implementációja. A soros portból érkezõ adatokat a SerialUtil statikus osztály dolgozza fel, a kapott csomag egy-egy adattagját egy DataElement objektumba helyezi, mely a hibadiagnosztikát valósítja meg a benne található elemre.
...
Minden fogadott adat egy DataElement objektumban tárolódik, ebben 2 FIFO lista szerepel, egyik az ``A'', másik a ``B'' portból érkezõeknek. Mikor ``A''-ból érkezik egy, az AddA(double item) függvénye teszi bele az ``A'' FIFO végére, ugyanez a másik portra is érvényes. Mikor egy mûszer elkéri az értéket, melyet mutatni szeretne, akkor a GetData() függvényhívással megkapja. A hibakezelés ezen függvényekben van megoldva, mindkettõ FIFO sor rendelkezik egy hibaszámlálóval, mely különbözõ feltételek mellett nõ vagy csökken. A GetData() függvény aszerint, hogy mely sornak kisebb ez a hibaszámlálója, dönti el, hogy melyikbõl veszi ki az elemet.
TODO
